...
export default function Historico() {
  ...
  const totalPastels = pedidos.reduce(
    (sum, pedido) => sum + (pedido.itens || []).reduce((itemSum, item) => itemSum + item.quantidade, 0),
    0
  );
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <span className="ml-auto bg-muted text-muted-foreground text-sm font-black px-3 py-1 rounded-full">
          {t('common.pastelCount', { count: totalPastels })}
        </span>
      </div>
                    ...
                    <div className="flex w-full flex-col gap-3 text-left sm:flex-row sm:items-center sm:justify-between">
                      <div className="min-w-0">
                        ...
                        <div className="mt-1 text-xs font-semibold text-muted-foreground">
                          {t('common.pastelCount', { count: totalItens })}
                        </div>
                      </div>
                ...
                return (
                  <div key={pedido.id} className="flex items-center justify-between bg-card rounded-xl border border-border px-4 py-3 shadow-sm">
                    <div>
                      ...
                      <span className="ml-2 text-xs text-muted-foreground font-semibold">{t('common.pastelCount', { count: total })}</span>
                    </div>
                    <Button
                      size="sm"
